...
<style>
    ...
    .warn-hint { font-size: 0.78rem; color: #d97706; margin-top: 4px; }
...
<div class="page-header">
    ...
    <h1>Editar Bus — {{ $bus->placa }}</h1>
</div>

<div class="form-card">
    <form method="POST" action="{{ route('buses.update', $bus->id_bus) }}">
        @csrf @method('PUT')
        ...
        <div class="form-row">
            ...
            <div class="form-group">
                ...
                <input type="number" name="capacidad" id="capacidad"
                    value="{{ old('capacidad', $bus->capacidad) }}" min="1" max="80"
                    class="{{ $errors->has('capacidad') ? 'is-invalid' : '' }}">
                <div class="warn-hint">Cambiar la capacidad regenera todos los asientos.</div>
                ...
                <select name="id_propietario" id="id_propietario"
                    ...
                    @foreach($propietarios as $propietario)
                        <option value="{{ $propietario->id_propietario }}"
                            {{ old('id_propietario', $bus->id_propietario) == $propietario->id_propietario ? 'selected' : '' }}>
                ...
                <select name="id_tipo_bus" id="id_tipo_bus"
                    ...
                    @foreach($tiposBus as $tipo)
                        <option value="{{ $tipo->id_tipo_bus }}"
                            ...
                            {{ old('id_tipo_bus', $bus->id_tipo_bus) == $tipo->id_tipo_bus ? 'selected' : '' }}>
                            {{ $tipo->nombre }}
