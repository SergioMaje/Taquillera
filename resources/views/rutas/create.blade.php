                ...
                <select name="id_municipio_origen" id="id_municipio_origen"
                    ...
                    @foreach($municipios as $mun)
                        <option value="{{ $mun->id_municipio }}"
                            {{ old('id_municipio_origen') == $mun->id_municipio ? 'selected' : '' }}>
                            {{ $mun->nombre }}
                        </option>
                    @endforeach
                </select>
        ...
        <div class="form-stack">
            <div class="form-group">
                ...
                <select name="id_municipio_destino" id="id_municipio_destino"
                    ...
                    {{ old('id_municipio_origen') ? '' : 'disabled' }}>
                    <option value="">— Seleccione primero el origen —</option>
                    @foreach($municipios as $mun)
                        <option value="{{ $mun->id_municipio }}"
                            {{ old('id_municipio_destino') == $mun->id_municipio ? 'selected' : '' }}>
                            {{ $mun->nombre }}
                        </option>
                    @endforeach
                </select>
...
@push('scripts')
<script>
    const selectOrigen  = document.getElementById('id_municipio_origen');
    const selectDestino = document.getElementById('id_municipio_destino');

    function actualizarDestino() {
        const origenId = selectOrigen.value;

        // Habilitar/deshabilitar destino
        selectDestino.disabled = !origenId;

        // Actualizar placeholder
        selectDestino.options[0].textContent = origenId
            ? '— Seleccione un municipio —'
            : '— Seleccione primero el origen —';

        // Mostrar/ocultar opciones según si es el mismo municipio que el origen
        Array.from(selectDestino.options).forEach(function (opt) {
            if (!opt.value) return;
            opt.hidden = opt.value === origenId;
        });

        // Si el destino seleccionado quedó oculto, limpiar selección
        if (selectDestino.value === origenId) {
            selectDestino.value = '';
        }
    }

    selectOrigen.addEventListener('change', actualizarDestino);

    // Aplicar al cargar (por errores de validación con old())
    actualizarDestino();
</script>
@endpush
